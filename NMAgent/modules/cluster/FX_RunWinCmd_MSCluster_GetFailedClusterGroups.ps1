##############################################################################
# Module:  FX_RunWinCmd_MSCluster_GetFailedClusterGroups
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_MSCluster_GetFailedClusterGroups = $true, $false, $true, 'System', 'FX_RunWinCmd_MSCluster_GetFailedClusterGroups', $false, @(); }


function FX_RunWinCmd_MSCluster_GetFailedClusterGroups([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      if ( $NodeData -ne $null ) {
        $ClusterGroups   = $( $NodeData | Select-String "failed|offline|pending|partially" | Measure-Object ).Count
