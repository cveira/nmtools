##############################################################################
# Module:  FX_RunWinCmd_SAN_HasMultiPath_EMC
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_SAN_HasMultiPath_EMC = $true, $false, $true, 'System', 'FX_RunWinCmd_SAN_HasMultiPath_EMC', $false, @(); }


function FX_RunWinCmd_SAN_HasMultiPath_EMC([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      if ( $NodeData -ne $null ) {
        $TotalPaths  = $( $NodeData | Select-String "failed|optimal" | Measure-Object ).Count
