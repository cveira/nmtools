##############################################################################
# Module:  FX_Windows_HasUnknownProcesses
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_Windows_HasUnknownProcesses = $true, $false, $true, 'System', 'FX_Windows_HasUnknownProcesses', $false, @(); }


function FX_Windows_HasUnknownProcesses([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $NodeData -ne $null ) {
      ...
      if ( $ResultValue -ne $null ) {
        $HasUnknownProcess = "YES"
      } else {
        $HasUnknownProcess = "NO"
      }

      $ErrorFound   = $false
      $ErrorText    = $( $NodeErrors | Format-List -force * | Out-String )
      $RawOutput    = $( $ResultValue | Out-String ).Trim()
      $ResultValue  = $HasUnknownProcess

      Write-Host -foregroundcolor $COLOR_RESULT "    + HasUnknownProcesses: " $HasUnknownProcess
    } else {
