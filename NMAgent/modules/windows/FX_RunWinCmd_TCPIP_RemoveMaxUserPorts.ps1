##############################################################################
# Module:  FX_RunWinCmd_TCPIP_RemoveMaxUserPorts
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_TCPIP_RemoveMaxUserPorts = $true, $false, $true, 'System', 'FX_RunWinCmd_TCPIP_RemoveMaxUserPorts', $false, @(); }


function FX_RunWinCmd_TCPIP_RemoveMaxUserPorts([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "reg delete HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters /v MaxUserPort /f"
