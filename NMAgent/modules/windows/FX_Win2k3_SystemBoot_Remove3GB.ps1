##############################################################################
# Module:  FX_Win2k3_SystemBoot_Remove3GB
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_Win2k3_SystemBoot_Remove3GB = $true, $true, $true, 'System', 'FX_Win2k3_SystemBoot_Remove3GB', $false, @(); }


function FX_Win2k3_SystemBoot_Remove3GB([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $ConnectionStatus -eq $OPERATION_SUCCESSFUL ) {
      ...
      if ( $NodeData -ne $null ) {
        ...
        if ( $( $NodeData | Select-String "PAE|3GB" ) -ne $null ) {
          $NewBootSettings =  $NodeData        -replace "/PAE",""
          $NewBootSettings =  $NewBootSettings -replace "/3GB",""
