##############################################################################
# Module:  FX_Windows_GetSecondaryDnsResolver.ps1
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_Windows_GetSecondaryDnsResolver = $true,  $false, $true, 'System', 'FX_Windows_GetSecondaryDnsResolver', $false, @(); }


function FX_Windows_GetSecondaryDnsResolver([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  . $ModulesDir\_SetModuleContext.ps1


  $DNS_SECONDARY_RESOLVER_INDEX = 1
      ...
      if ( $NodeData -ne $null ) {
        ...
        $AdapterInfo += $NodeData | ForEach-Object {
          New-Object PSObject -Property @{
            ...
            Value              = $_.DNSServerSearchOrder[$DNS_SECONDARY_RESOLVER_INDEX]
          }
        }
        ...
        Write-Host -foregroundcolor $COLOR_RESULT "    + SecondaryDnsResolver:"
        Write-Host
