##############################################################################
# Module:  FX_RunWinCmd_SAN_HasMultiPath_IBM
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_SAN_HasMultiPath_IBM = $true, $false, $true, 'System', 'FX_RunWinCmd_SAN_HasMultiPath_IBM', $false, @(); }


function FX_RunWinCmd_SAN_HasMultiPath_IBM([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      if ( $NodeData -ne $null ) {
        $TotalPaths  = $($NodeData | Select-String "NORMAL|FAILED" | Measure-Object).Count
