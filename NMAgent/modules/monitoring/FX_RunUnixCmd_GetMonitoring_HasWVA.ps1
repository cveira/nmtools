##############################################################################
# Module:  FX_RunUnixCmd_GetMonitoring_HasWVA
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunUnixCmd_GetMonitoring_HasWVA = $true, $false, $true, 'System', 'FX_RunUnixCmd_GetMonitoring_HasWVA', $false, @(); }


function FX_RunUnixCmd_GetMonitoring_HasWVA([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PuTTYCmd        = "`"ps -ef | grep /WVA/MonSrv/jre/bin/java`""
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      ...
      Write-Host
      Write-Host -foregroundcolor $COLOR_RESULT "    + HasWVA:              " $ResultValue
    } else {
