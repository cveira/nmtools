##############################################################################
# Module:  FX_RunUnixCmd_GetMonitoring_HasAll
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunUnixCmd_GetMonitoring_HasAll = $true, $false, $true, 'System', 'FX_RunUnixCmd_GetMonitoring_HasAll', $false, @(); }


function FX_RunUnixCmd_GetMonitoring_HasAll([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PuTTYCmd        = "`"ps -ef | egrep 'opcctla|kux|/WVA/MonSrv/jre/bin/java' | grep -v egrep`""
