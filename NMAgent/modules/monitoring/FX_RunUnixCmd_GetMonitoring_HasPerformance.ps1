##############################################################################
# Module:  FX_RunUnixCmd_GetMonitoring_HasPerformance
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunUnixCmd_GetMonitoring_HasPerformance = $true, $false, $true, 'System', 'FX_RunUnixCmd_GetMonitoring_HasPerformance', $false, @(); }


function FX_RunUnixCmd_GetMonitoring_HasPerformance([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PuTTYCmd        = "`"ps -ef | egrep 'scopeux|omv_bdc' | grep -v egrep`""
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      ...
      Write-Host
      Write-Host -foregroundcolor $COLOR_RESULT "    + HasPerformance:      " $ResultValue
    } else {
