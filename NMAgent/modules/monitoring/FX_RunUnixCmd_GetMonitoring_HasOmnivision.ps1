##############################################################################
# Module:  FX_RunUnixCmd_GetMonitoring_HasOmnivision
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunUnixCmd_GetMonitoring_HasOmnivision = $true, $false, $true, 'System', 'FX_RunUnixCmd_GetMonitoring_HasOmnivision', $false, @(); }


function FX_RunUnixCmd_GetMonitoring_HasOmnivision([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $SavedExitCode -eq $SUCCESS_EXITCODE ) {
      ...
      Write-Host
      Write-Host -foregroundcolor $COLOR_RESULT "    + HasOmniVision:       " $ResultValue
    } else {
