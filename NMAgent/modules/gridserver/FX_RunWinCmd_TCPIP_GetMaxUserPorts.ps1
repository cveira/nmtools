##############################################################################
# Module:  FX_RunWinCmd_TCPIP_GetMaxUserPorts
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_TCPIP_GetMaxUserPorts = $true, $false, $true, 'System', 'FX_RunWinCmd_TCPIP_GetMaxUserPorts', $false, @(); }


function FX_RunWinCmd_TCPIP_GetMaxUserPorts([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "reg query HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters /f MaxUserPort"
