##############################################################################
# Module:  FX_RunWinCmd_GP_DisableLocalAccountTokenFilterPolicy
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_GP_DisableLocalAccountTokenFilterPolicy = $true, $false, $true, 'System', 'FX_RunWinCmd_GP_DisableLocalAccountTokenFilterPolicy', $false, @(); }


function FX_RunWinCmd_GP_DisableLocalAccountTokenFilterPolicy([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "reg delete HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System /v LocalAccountTokenFilterPolicy /f"
