##############################################################################
# Module:  PLEX_RunWinCmd_WSMan_ServicePolicy_SetIP6Filter
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ PLEX_RunWinCmd_WSMan_ServicePolicy_SetIP6Filter = $true, $false, $true, 'System', 'PLEX_RunWinCmd_WSMan_ServicePolicy_SetIP6Filter', $false, @(); }


function PLEX_RunWinCmd_WSMan_ServicePolicy_SetIP6Filter([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service @{IPv6Filter=#$PipelineInput#}`"" -replace '#', '`"'
