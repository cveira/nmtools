##############################################################################
# Module:  FX_RunWinCmd_WSMan_GetServicePolicy
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_GetServicePolicy = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_GetServicePolicy', $false, @(); }


function FX_RunWinCmd_WSMan_GetServicePolicy([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "$RemoteSystemDir\winrm.cmd get winrm/config/service"
