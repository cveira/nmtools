##############################################################################
# Module:  FX_RunWinCmd_GP_EnableLocalAccountTokenFilterPolicy
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_GP_EnableLocalAccountTokenFilterPolicy = $true, $false, $true, 'System', 'FX_RunWinCmd_GP_EnableLocalAccountTokenFilterPolicy', $false, @(); }


function FX_RunWinCmd_GP_EnableLocalAccountTokenFilterPolicy([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "reg add HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System /v LocalAccountTokenFilterPolicy /t REG_DWORD /d 1 /f"
