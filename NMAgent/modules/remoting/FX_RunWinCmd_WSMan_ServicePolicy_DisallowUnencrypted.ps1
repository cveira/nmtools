##############################################################################
# Module:  FX_RunWinCmd_WSMan_ServicePolicy_DisallowUnencrypted
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ServicePolicy_DisallowUnencrypted = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ServicePolicy_DisallowUnencrypted', $false, @(); }


function FX_RunWinCmd_WSMan_ServicePolicy_DisallowUnencrypted([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service @{AllowUnencrypted=#false#}`"" -replace '#', '`"'
