##############################################################################
# Module:  FX_RunWinCmd_WSMan_ServicePolicy_DisableBasicAuth
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ServicePolicy_DisableBasicAuth = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ServicePolicy_DisableBasicAuth', $false, @(); }


function FX_RunWinCmd_WSMan_ServicePolicy_DisableBasicAuth([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service/auth @{Basic=#false#}`"" -replace '#', '`"'
