##############################################################################
# Module:  FX_RunWinCmd_WSMan_ServicePolicy_AllowUnencrypted
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ServicePolicy_AllowUnencrypted = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ServicePolicy_AllowUnencrypted', $false, @(); }


function FX_RunWinCmd_WSMan_ServicePolicy_AllowUnencrypted([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service @{AllowUnencrypted=#true#}`"" -replace '#', '`"'
