##############################################################################
# Module:  FX_RunWinCmd_WSMan_ServicePolicy_DisableIP6Listeners
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ServicePolicy_DisableIP6Listeners = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ServicePolicy_DisableIP6Listeners', $false, @(); }


function FX_RunWinCmd_WSMan_ServicePolicy_DisableIP6Listeners([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service @{IPv6Filter=##}`"" -replace '#', '`"'
