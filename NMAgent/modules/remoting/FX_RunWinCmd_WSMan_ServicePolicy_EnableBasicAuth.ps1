##############################################################################
# Module:  FX_RunWinCmd_WSMan_ServicePolicy_EnableBasicAuth
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ServicePolicy_EnableBasicAuth = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ServicePolicy_EnableBasicAuth', $false, @(); }


function FX_RunWinCmd_WSMan_ServicePolicy_EnableBasicAuth([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/service/auth @{Basic=#true#}`"" -replace '#', '`"'
