##############################################################################
# Module:  FX_RunWinCmd_WSMan_ClientPolicy_TrustAllHosts
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ClientPolicy_TrustAllHosts = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ClientPolicy_TrustAllHosts', $false, @(); }


function FX_RunWinCmd_WSMan_ClientPolicy_TrustAllHosts([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad    = "`"winrm.cmd set winrm/config/client @{TrustedHosts=#*#}`"" -replace '#', '`"'
