##############################################################################
# Module:  FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpPort
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpPort = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpPort', $false, @(); }


function FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpPort([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/client/DefaultPorts @{HTTP=#5985#}`"" -replace '#', '`"'
