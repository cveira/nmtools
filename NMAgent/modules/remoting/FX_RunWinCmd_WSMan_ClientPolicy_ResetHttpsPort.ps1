##############################################################################
# Module:  FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpsPort
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpsPort = $true, $false, $true, 'System', 'FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpsPort', $false, @(); }


function FX_RunWinCmd_WSMan_ClientPolicy_ResetHttpsPort([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecPayLoad  = "`"winrm.cmd set winrm/config/client/DefaultPorts @{HTTPS=#5986#}`"" -replace '#', '`"'
