##############################################################################
# Module:  FX_RunWinCmd_GP_RefreshPolicies
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ FX_RunWinCmd_GP_RefreshPolicies = $true, $false, $true, 'System', 'FX_RunWinCmd_GP_RefreshPolicies', $false, @(); }


function FX_RunWinCmd_GP_RefreshPolicies([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  $PsExecCmd        = "gpupdate /force /wait:-1"
