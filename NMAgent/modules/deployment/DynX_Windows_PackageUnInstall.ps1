##############################################################################
# Module:  DynX_Windows_PackageUnInstall
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ DynX_Windows_PackageUnInstall = $true, $false, $false, 'System', 'DynX_Windows_PackageUnInstall', $false, @(); }


function DynX_Windows_PackageUnInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  Get-ChildItem $ModulesDir\$CurrentProfile\DynX_Windows_PackageUnInstall\*.ps1 -exclude _* | ForEach-Object {
  ...
  if ( ( $PackagesToInstall -ne $null ) -and ( $FileSetSize -ne 0 ) -and ( !$SkipCurrentNode ) ) {
    if ( $SystemType -ne $UNKNOWN_ITEM ) {
      ...
      if ( !$SkipCurrentNode ) {
        ...
        Write-Host
        . ReportModuleError $InstalledOS "INFO: Skipping Host. Problems uploading files to the Remote System."
      }
    } else {
      Write-Host
      . ReportModuleError $InstalledOS "INFO: Skipping Host. Unknown System Type."
    }
  } else {
    Write-Host
    . ReportModuleError $InstalledOS "INFO: Skipping Host. No Packages to Deploy."
  }
