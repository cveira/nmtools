##############################################################################
# Module:  DynX_Microsoft_IExpressInstall
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ DynX_Microsoft_IExpressInstall = $true, $false, $false, 'System', 'DynX_Microsoft_IExpressInstall', $false, @(); }


function DynX_Microsoft_IExpressInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  Get-ChildItem $ModulesDir\$CurrentProfile\DynX_Microsoft_IExpressInstall\*.ps1 -exclude _* | ForEach-Object {
      ...
      if ( !$SkipHost ) {
        ...
        if ( $PackagesToInstall -ne $null ) {
          $PackagesToInstall | ForEach-Object {
            ...
            DynX1_RunWinCmd_Microsoft_IExpressInstall $SetConnectToHostByIP $NodeName $NodeIP "PLEX_RunWinCmd_Microsoft_PackageInstall" $IsSupported $IsString
          }
        } else {
