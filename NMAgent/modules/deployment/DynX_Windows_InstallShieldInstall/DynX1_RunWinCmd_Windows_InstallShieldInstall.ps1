...
function DynX1_RunWinCmd_Windows_InstallShieldInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  # $PsExecPayLoad    = "-s -f1\\$SX_RepositoryServer\$SX_SoftwareStore\$SX_SoftwareLibrary\$SystemType\$($CurrentPackage.Name.Split('.')[0] + '.iss') -f2C:\$SX_SoftwareLibrary-$SystemType-$LogSessionId.log"
  $PsExecPayLoad    = "-s -f2C:\$SX_SoftwareLibrary-$SystemType-$LogSessionId.log"
