##############################################################################
# Module:  DynX_Microsoft_DotNetFwInstall
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ DynX_Microsoft_DotNetFwInstall = $true, $false, $false, 'System', 'DynX_Microsoft_DotNetFwInstall', $false, @(); }


function DynX_Microsoft_DotNetFwInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  Get-ChildItem $ModulesDir\$CurrentProfile\DynX_Microsoft_DotNetFwInstall\*.ps1 -exclude _* | ForEach-Object {
  ...
  if ( ( $InstalledOS -ne $null ) -and ( $ProcessorInfo -ne $null ) -and ( !$SkipCurrentNode ) ) {
    ...
    if ( !$IsUnknown ) {
      ...
      if ( !$SkipHost ) {
        $PackagesToInstall = Get-ChildItem \\$SX_RepositoryServer\$SX_SoftwareStore\$SX_SoftwareLibrary\$SystemType\* -include *.msi -exclude _* | Sort-Object Name
        ...
        if ( $PackagesToInstall -ne $null ) {
          $PackagesToInstall | ForEach-Object {
            ...
            DynX1_RunWinCmd_Microsoft_DotNetFwInstall $SetConnectToHostByIP $NodeName $NodeIP "PLEX_RunWinCmd_Microsoft_PackageInstall" $IsSupported $IsString
          }
        } else {
