...
function DynX1_RunWinCmd_Windows_InnoSetupInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  # $PsExecPayLoad    = "/sp- /verysilent /norestart /loadinf=`"\\$SX_RepositoryServer\$SX_SoftwareStore\$SX_SoftwareLibrary\$SystemType\$($CurrentPackage.Name.Split('.')[0] + '.inf')`" /log=C:\$SX_SoftwareLibrary-$SystemType-$LogSessionId.log"
  $PsExecPayLoad    = "/sp- /verysilent /norestart /log=C:\$SX_SoftwareLibrary-$SystemType-$LogSessionId.log"
