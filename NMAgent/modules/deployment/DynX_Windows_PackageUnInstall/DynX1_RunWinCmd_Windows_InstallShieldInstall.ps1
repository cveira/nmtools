##############################################################################
# Module:  DynX1_RunWinCmd_Windows_InstallShieldInstall
# Version: 4.54b0
# Date:    2010/12/11
# Author:  Carlos Veira Lorenzo - cveira [at] thinkinbig [dot] org
##############################################################################

function DynX1_RunWinCmd_Windows_InstallShieldInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  # $PsExecPayLoad    = "-s -f1C:\NMAWorkArea\$LogSessionId\$($CurrentPackageShortName + '.iss') -f2C:\$CurrentPackageShortName-$SystemType-$LogSessionId.log"
  # $PsExecPayLoad    = "-s -f1\\$SX_RepositoryServer\$SX_SoftwareStore\$SX_SoftwareLibrary\$SystemType\$($CurrentPackageShortName + '.iss') -f2C:\$CurrentPackageShortName-$SystemType-$LogSessionId.log"
  $PsExecPayLoad    = "-s -f2C:\$CurrentPackageShortName-$SystemType-$LogSessionId.log"
