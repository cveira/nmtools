##############################################################################
# Module:  DynX_Microsoft_SfxCabInstall
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ DynX_Microsoft_SfxCabInstall = $true, $false, $false, 'System', 'DynX_Microsoft_SfxCabInstall', $false, @(); }


function DynX_Microsoft_SfxCabInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  Get-ChildItem $ModulesDir\$CurrentProfile\DynX_Microsoft_SfxCabInstall\*.ps1 -exclude _* | ForEach-Object {
      ...
      if ( !$SkipHost ) {
        ...
        if ( $PackagesToInstall -ne $null ) {
          $PackagesToInstall | ForEach-Object {
            ...
            DynX1_RunWinCmd_Microsoft_SfxCabInstall $SetConnectToHostByIP $NodeName $NodeIP "PLEX_RunWinCmd_Microsoft_PackageInstall" $IsSupported $IsString
          }
        } else {
