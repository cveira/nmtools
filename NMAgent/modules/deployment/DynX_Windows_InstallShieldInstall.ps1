##############################################################################
# Module:  DynX_Windows_InstallShieldInstall
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ DynX_Windows_InstallShieldInstall = $true, $false, $false, 'System', 'DynX_Windows_InstallShieldInstall', $false, @(); }


function DynX_Windows_InstallShieldInstall([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  Get-ChildItem $ModulesDir\$CurrentProfile\DynX_Windows_InstallShieldInstall\*.ps1 -exclude _* | ForEach-Object {
      ...
      if ( !$SkipHost ) {
        ...
        if ( $PackagesToInstall -ne $null ) {
          $PackagesToInstall | ForEach-Object {
            ...
            DynX1_Windows_InstallShieldInstall $SetConnectToHostByIP $NodeName $NodeIP "PLEX_RunWinCmd_Microsoft_PackageInstall" $IsSupported $IsString
          }
        } else {
