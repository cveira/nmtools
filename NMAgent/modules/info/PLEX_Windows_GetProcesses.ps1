##############################################################################
# Module:  PLEX_Windows_GetProcesses
# Version: 4.54b0
# Date:    2010/12/11
...
$script:LibraryModules += @{ PLEX_Windows_GetProcesses = $true, $false, $false, 'System', 'PLEX_Windows_GetProcesses', $false, @(); }


function PLEX_Windows_GetProcesses([bool] $SetConnectToHostByIP, [string] $NodeName, [string] $NodeIP, [string] $PropertyName, [bool] $IsSupported, [bool] $IsString) {
  ...
  if ( $test ) {
    ...
    if ( $NodeData -ne $null ) {
      $SelectedProcesses = $NodeData | Where-Object { ( $_.Name -ne $null ) -and ( $_.CommandLine -match "$PipeLineInput" ) }
